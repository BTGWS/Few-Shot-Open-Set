...
class OmniglotDataset(data.Dataset):
    ...
    ## Uses splits from Matching Nets paper. Define your anomaly classes in the form of .txt files
    ...
    def __init__(self, mode='train', root='..' + os.sep + 'dataset', transform=None, resize=[28,28], target_transform=None, download=True):
        '''
        ...
        if not transform:
            transform = torchvision.transforms.Compose([torchvision.transforms.ToPILImage(),
                                                  torchvision.transforms.Resize((resize[1],resize[0]),interpolation=Image.BICUBIC),
                                                  torchvision.transforms.ToTensor()])

        self.transform = transform
        self.template_transform = torchvision.transforms.Compose([torchvision.transforms.Resize((resize[1],resize[0]),interpolation=Image.BICUBIC),
                                                  torchvision.transforms.ToTensor()])
        ...
        self.x = list(self.x)
        self.cls = list(set(self.y))
        self.targets = self.y
        self.indices = get_indices(self.y, self.cls)
        assert len(self.cls) == len(self.indices.keys())

    def __getitem__(self, idx):
        template = Image.open(self.root + 'templates/'+self.mode+'/class_tensor('+str(self.y[idx])+')')
        x = self.x[idx]
        if self.transform:
            x = self.transform(x)
