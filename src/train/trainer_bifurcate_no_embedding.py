...
def train(model,device,train_loader,val_loader,tester,opts):
    ...
    # optimizer = torch.optim.SGD(model.enc_module.parameters(), lr=LR, momentum=0.9, weight_decay=0.0005)
    if len(sch) != 0:
        ...
        for i,episode in enumerate(train_loader):
            ...
            for k in classes:
                 ...
                 if(backbone == "MLP"):
                    ...
                    l_vpe_s = l_vpe_s + loss_vpe(recon,s,mu,log_var,device,recon = recon_loss)
                 else:
                    l_vpe_s = l_vpe_s + loss_vpe(recon,symbolic_proto_k[:n_support,:,:,:],mu,log_var,device,recon = recon_loss)
